...
color3 specularReflect(Ray* ray, Intersection* intersection, Scene* scene, KdTree* tree, Sampler* sampler){
  ...
  // BRDF
  color3 f = intersection->mat->sample_f(ray->dir, &wi, intersection->normal, point2(0,0)/*sampler->Get2D()*/, &pdf, type);

  if(pdf > 0 && !isBlack(f)) {
    vec3 normal = intersection->isOutside ? intersection->normal : -intersection->normal;
    Ray ray_ref;
    rayInit(&ray_ref, intersection->position + (acne_eps * wi), wi, ray->pixel, 
            0, 100000, ray->depth + 1);
...
color3 specularTransmission(Ray* ray, Intersection* intersection, Scene* scene, KdTree* tree, Sampler* sampler){
  ...
  if(pdf > 0 && !isBlack(f)) {
    vec3 normal = intersection->isOutside ? intersection->normal : -intersection->normal;
    Ray ray_refr;
    rayInit(&ray_refr, intersection->position + (acne_eps * wi), wi, ray->pixel,0, 100000, ray->depth + 1);
    if(ray->hasDifferentials){
      vec3 dndx = intersection->dn[0] * intersection->dudx +
                  intersection->dn[1] * intersection->dvdx;
      vec3 dndy = intersection->dn[0] * intersection->dudy +
