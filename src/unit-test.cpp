...
int main( void ) {
    ...
    rayInit( &r, point3( 2, 2, 2 ), vec3( -1, -1, 0 - 1 ) );
    validTest( "r2 to plane1", intersectPlane( &r, &dummyInter, plane1 ), true );
    rayInit( &r, point3( 2, 2, 2 ), vec3( -1, -1, 0 - 1 ) );
    validTest( "r2 to plane2", intersectPlane( &r, &dummyInter, plane2 ), true );
    ...
    for ( int i = 0; i < beckmannExpectedCount; i++ ) {
        beckmann &=
            abs( beckmannExpected[i].res -
                 RDM_Beckmann( beckmannExpected[i].NdotH, beckmannExpected[i].alpha ) ) < 0.0001f;
    }
    ...
    for ( int i = 0; i < fresnelExpectedCount; i++ ) {
        fresnel &=
            abs( fresnelExpected[i].res - RDM_Fresnel( fresnelExpected[i].LdotH,
                                                       fresnelExpected[i].extIOR,
                                                       fresnelExpected[i].intIOR ) ) < 0.0001f;
    }
