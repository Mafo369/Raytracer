...
color3 RDM_btdf( float LdotH,
    ...
    float no = intIOR;
    return color3( specularColor * ( LdotH * VdotH ) / ( LdotN * VdotN ) *
                   ( ( no * no ) * ( 1.0f - f ) * g * d ) / ( denom * denom ) );
}
...
color3 RDM_brdf( float LdotH,
                 ...
                 float roughness,
                 color3 specularColor ) {
    if ( VdotN == 0.f ) return color3( 0, 0, 0 );
...
static void
BeckmannSample11( float cosThetaI, float U1, float U2, float* slope_x, float* slope_y ) {
    ...
    while ( ++it < 10 ) {
        ...
        float value =
            normalization * ( 1 + b + SQRT_PI_INV * tanThetaI * std::exp( -invErf * invErf ) ) -
            sample_x;
...
static vec3 BeckmannSample( const vec3& wi, float alpha, float U1, float U2, const vec3& normal ) {
    // 1. stretch wi
    vec3 wiStretched = normalize( vec3( alpha * wi.x, alpha * wi.y, wi.z ) );
...
float BeckmannPdf( const vec3& wo,
    ...
    return RDM_Beckmann( NdotH, roughness ) * RDM_Smith( LdotH, LdotN, VdotH, VdotN, roughness ) *
           abs( dot( wo, wh ) ) / abs( VdotN );
}
