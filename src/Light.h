...
class Light
{
  public:
    ...
    virtual float intensityAt( vec3 point,
                               Scene* scene,
                               KdTree* tree,
                               vec3 view,
                               Intersection* intersection ) = 0;
    virtual std::vector<vec3> getSamples() { return m_samples; }
    virtual color3 sample_Li( Scene* scene,
                              KdTree* tree,
                              const Intersection& inter,
                              const point2& u,
                              vec3* wi,
                              float* pdf,
                              bool* visibility ) const                   = 0;
    virtual float pdf_Li( const Intersection& it, const vec3& wi ) const = 0;
    virtual color3 Le( Ray* ray ) const { return color3( 0 ); }
...
{
  public:
    ShapeLight( vec3 position, color3 color, Object* obj ) : Light() {
        m_shape    = obj;
        m_position = position;
        m_color    = color;
    }
    ...
    IBL( const std::string& filename ) {
        ...
        for ( int v = 0; v < m_height - 3; v += 3 ) {
            ...
            for ( int u = 0; u < m_width - 3; u += 3 ) {
                ...
                img[u + v * m_width] = luminance( color3( m_pixels[u + v * m_width],
                                                          m_pixels[u + v * m_width + 1],
                                                          m_pixels[u + v * m_width + 2] ) );
